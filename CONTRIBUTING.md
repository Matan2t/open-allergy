...
2. Find your language's section (keyed by language code, e.g. `el:` for Greek).
...
2. Fill in the English (`en`) entry first — it is the source text for all translations.
...
1. Add the language to `data/languages.yaml` with its code ([ISO 639-1](https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes)),
   English name, native name, text direction (`ltr` or `rtl`), and the shared UI strings
   (card title, caution box text, etc.).
2. Add a translation section under that code to each file in `data/allergens/` — partial
